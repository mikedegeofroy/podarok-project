...
function ChildrenList(data){
    ...
    {children.map(({ name, gender, slug, status }, index) => { 
        ...
        return(
            <Link key={index} href={`/dashboard/${slug}`}><div className={`text-center h-full w-full rounded-lg p-4 ${color}`}><h2 className='text-3xl'>{name}</h2><Image src={source} height={'278px'} width={'130px'}/><div>{status}</div></div></Link>
        )
    })}
    {children.length < 4 && 
